...
## Requirements
1. Please download the appropriate [artifact](https://github.com/cole-gannaway/go-cloc/releases) for your platform and simply run the single exectuable in PowerShell or a terminal. Either add the binary to your PATH or explicityly specify the path to the binary, like so:
```sh
# Linux
...
# If added to PATH, you can run the binary directly without specifying the path.
go-cloc -h
```
...
go-cloc folder --csv-file-path results.csv
```
...
The CSV reports provide a structured way to store the results of your code analysis, which can be useful for further processing or integration with other tools. Here is an example of what the CSV report might look like:
```csv
...
-  `-html-reports-directory-path`
        Path to dumps HTML reports into a specified directory, otherwise HTML reports are not generated.
-  `-ignore-file-path`
